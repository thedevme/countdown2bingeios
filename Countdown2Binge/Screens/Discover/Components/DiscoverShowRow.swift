//
//  DiscoverShowRow.swift
//  Countdown2Binge
//
//  Row-style show card for timeline view with episode count, poster, network, and follow button.
//

import SwiftUI

struct DiscoverShowRow: View {
    let show: ShowSummary
    let network: NetworkDefinition
    let episodeCount: Int?
    let seasonCount: Int?
    let isFollowing: Bool
    let onTap: () -> Void
    let onFollowTap: () -> Void

    private var posterURL: URL? {
        guard let path = show.posterPath else { return nil }
        return URL(string: "https://image.tmdb.org/t/p/w154\(path)")
    }

    private var daysUntilAir: Int? {
        guard let dateString = show.firstAirDate,
              let date = parseDate(dateString) else { return nil }
        return Calendar.current.dateComponents([.day], from: Date(), to: date).day
    }

    private var isAvailableNow: Bool {
        guard let days = daysUntilAir else { return true }
        return days <= 0
    }

    private var isImminentPremiere: Bool {
        guard let days = daysUntilAir else { return false }
        return days > 0 && days <= 14
    }

    private var statusText: String {
        guard let dateString = show.firstAirDate,
              let date = parseDate(dateString) else {
            return "FULL SEASON AVAILABLE"
        }

        let days = daysUntilAir ?? 0

        if days <= 0 {
            return "FULL SEASON AVAILABLE"
        } else if days <= 7 {
            return "PREMIERES IN \(days) DAY\(days == 1 ? "" : "S")"
        } else {
            let formatter = DateFormatter()
            formatter.dateFormat = "MMM d"
            return "PREMIERES \(formatter.string(from: date).uppercased())"
        }
    }

    // Teal for available now or imminent (≤14 days), white for further out
    private var countColor: Color {
        if isAvailableNow || isImminentPremiere {
            return .c2bTeal
        }
        return .c2bText
    }

    var body: some View {
        Button(action: onTap) {
            HStack(spacing: 14) {
                // Episode count
                VStack(spacing: 2) {
                    Text("\(episodeCount ?? 0)")
                        .font(.custom(.oswald.bold, size: 28))
                        .foregroundColor(countColor)
                    Text("EPS")
                        .font(.custom(.jetbrains.regular, size: 9))
                        .foregroundColor(countColor)
                        .tracking(0.5)
                }
                .frame(width: 44)

                // Poster thumbnail
                if let url = posterURL {
                    AsyncImage(url: url) { phase in
                        switch phase {
                        case .success(let image):
                            image
                                .resizable()
                                .aspectRatio(contentMode: .fill)
                                .frame(width: 52, height: 78)
                                .clipShape(RoundedRectangle(cornerRadius: 8))
                        default:
                            posterPlaceholder
                        }
                    }
                } else {
                    posterPlaceholder
                }

                // Show info
                VStack(alignment: .leading, spacing: 6) {
                    // Title and season
                    HStack(alignment: .firstTextBaseline, spacing: 8) {
                        Text(show.name.uppercased())
                            .font(.custom(.oswald.bold, size: 17))
                            .foregroundColor(.c2bText)
                            .lineLimit(1)

                        if let seasons = seasonCount, seasons > 0 {
                            Text("S\(seasons)")
                                .font(.custom(.oswald.bold, size: 12))
                                .foregroundColor(.c2bMuted)
                        }
                    }

                    // Network and status
                    HStack(spacing: 6) {
                        Circle()
                            .fill(Color(hex: network.color))
                            .frame(width: 6, height: 6)

                        Text("\(network.name.uppercased()) \u{00B7} \(statusText)")
                            .font(.custom(.jetbrains.regular, size: 9))
                            .foregroundColor(.c2bMuted)
                            .tracking(0.5)
                            .lineLimit(1)
                    }
                }

                Spacer()

                // Follow button
                Button(action: onFollowTap) {
                    ZStack {
                        RoundedRectangle(cornerRadius: 10)
                            .fill(isFollowing ? Color.c2bTeal : Color.c2bSurface)
                            .frame(width: 44, height: 44)

                        Image(systemName: isFollowing ? "checkmark" : "bookmark.fill")
                            .font(.system(size: 16, weight: .semibold))
                            .foregroundColor(isFollowing ? Color(hex: "#04201c") : .c2bTeal)
                    }
                }
            }
            .padding(.horizontal, C2BLayout.horizontalPadding)
            .padding(.vertical, 12)
        }
        .buttonStyle(.plain)
    }

    private var posterPlaceholder: some View {
        RoundedRectangle(cornerRadius: 8)
            .fill(Color.c2bSurface)
            .frame(width: 52, height: 78)
            .overlay(
                Image(systemName: "tv")
                    .font(.system(size: 16))
                    .foregroundColor(.c2bMuted)
            )
    }

    private func parseDate(_ string: String) -> Date? {
        let formatter = DateFormatter()
        formatter.dateFormat = "yyyy-MM-dd"
        return formatter.date(from: string)
    }
}

#Preview {
    VStack(spacing: 0) {
        DiscoverShowRow(
            show: ShowSummary(
                id: 1,
                name: "Fallout",
                overview: "A post-apocalyptic drama",
                posterPath: nil,
                backdropPath: nil,
                firstAirDate: "2024-04-10",
                voteAverage: 8.5,
                genreIds: [18, 10765]
            ),
            network: NetworkDefinition(id: 1024, name: "Prime", color: "#1FB6FF"),
            episodeCount: 8,
            seasonCount: 1,
            isFollowing: false,
            onTap: {},
            onFollowTap: {}
        )

        DiscoverShowRow(
            show: ShowSummary(
                id: 2,
                name: "The White Lotus",
                overview: "A social satire",
                posterPath: nil,
                backdropPath: nil,
                firstAirDate: "2024-05-01",
                voteAverage: 8.2,
                genreIds: [18, 35]
            ),
            network: NetworkDefinition(id: 3186, name: "Max", color: "#5A35E0"),
            episodeCount: 7,
            seasonCount: 3,
            isFollowing: true,
            onTap: {},
            onFollowTap: {}
        )
    }
    .background(Color.c2bBackground)
    .preferredColorScheme(.dark)
}
