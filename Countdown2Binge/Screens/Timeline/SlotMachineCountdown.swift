...
struct SlotMachineCountdown: View {
    ...
    private var subtitle: String {
        switch displayMode {
        case .days: return String(localized: "countdown_episode_in")
        case .episodes: return String(localized: "countdown_in")
        }
    }

    private var accessibilityLabel: String {
        guard let value = value, value >= 0, value <= 99 else {
            return String(localized: "date_tbd")
        }
        let unit = displayMode == .days
            ? String(localized: value == 1 ? "time_day" : "time_days")
            : String(localized: value == 1 ? "time_episode" : "time_episodes")
        return "\(String(localized: "date_finale")) \(value) \(unit)"
    }

    var body: some View {
        VStack(spacing: 16) {
            VStack(spacing: -4) {
                Text(String(localized: "date_finale"))
...
private struct SlotMachineReel: View {
    ...
    var body: some View {
        ZStack {
            ...
            .drawingGroup()
            .offset(x: xOffset)
            .animation(reduceMotion ? .none : .easeOut(duration: 0.4), value: displayValue)
            .mask(
    ...
    private func numberCell(for number: Int) -> some View {
        VStack(spacing: -4) {
            if number == tbdIndex {
                Text(String(localized: "date_tbd"))
